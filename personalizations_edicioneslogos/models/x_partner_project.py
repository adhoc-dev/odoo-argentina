...
class XPartnerProject(models.Model):
    ...
    x_country_id = fields.Many2one(string="País", related="x_partner_id.country_id", on_delete="set null", copy=False, store=True)
    x_state = fields.Many2one(string="Provincia", related="x_partner_id.state_id", on_delete="set null", store=True)
    x_user_id = fields.Many2one(string="Comercial", related="x_partner_id.user_id", help="The internal user that is in charge of communicating with this contact if any.", on_delete="set null", store=True)
